#################################################################################################################
#################################################################################################################
### A.Read_data/A2.Descriptives_prm.R
...
rm(list=ls())
.res <- paste0(.res0, "A.Read_data/A2.Descriptives_prm/")
dir.create(.res, F, T)

#################################################################################################################
  # Descriptives for PRM series - All
  # - No previous infection

rm(list=ls())
nd <- readRDS(paste0(.res0, "O.Concordance_DIA_PRM_complete/O1.Merge_PRM_data/nd.prm.all.rds"))
ndd <- readRDS(paste0(.dat, "1_DIA/nd.fp.rds"))
...
v <- c("dm","aht","obesity","dlp")
aux <- d[,v]
o <- aux[2,]

cvrf <- apply(aux,1,function(o){
  if(all(is.na(o))){return(NA)}
  sum(o[!is.na(o)]=="Yes")
})
# aux[is.na(cvrf),]
d$cvrf <- cvrf[rownames(d)]


ny <- "group"
nxs <- c("sex", "age",
         "center", "previous.covid", "vaccinated", 
         "aht", "dm", "obesity", "dlp","cvrf",
...
names(nv) <- c("Sex", "Age at diagnosi",
               ...
               "Arterial Hypertension", "Diabetes Mellitus", "Obesity", "Dyslipidemia","Number of CVRF",
...
p <- openPage(paste0(.res,  "Descriptives_prm_all.html"))
hwrite(paste0('<br><br>Descriptives - Univariate association with condition group - PRM<br><br>',
