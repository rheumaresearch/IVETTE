#################################################################################################################
#################################################################################################################
### O.Concordance_DIA_PRM_complete/O2.Diff_exprs_hosp_PRM.R
...
rm(list=ls())
.res <- paste0(.res0, "O.Concordance_DIA_PRM_complete/O2.Diff_exprs_hosp_PRM/")
dir.create(.res, F, T)
...
rm(list=ls())
nd <- readRDS(paste0(.dat, "4_Concordance_DIA.PRM/nd.prm.all.rds"))
nd2 <- readRDS(paste0(.dat, "1_DIA/nd.fp.rds"))
...
nds <- nds[regexpr("iRT-Kit_WR_fusion", featureNames(nds)) < 0, ]
