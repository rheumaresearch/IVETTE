#################################################################################################################
#################################################################################################################
### G.Discovery_DIA_FP_MaxLFQ/G5.ROAST_GSA.R
...
rm(list=ls())
.res <- paste0(.res0, "G.Discovery_DIA_FP_MaxLFQ/G5.ROAST_GSA/")
dir.create(.res, F, T)
...
nd <- nd[!is.na(fData(nd)$gene.symbol) & fData(nd)$gene.symbol!='NA', ]

featureNames(nd) <- fData(nd)$gene.symbol
...
ndr <- new('ExpressionSet', exprs=Mr, featureData=new('AnnotatedDataFrame', fData(nd)),
           phenoData=new('AnnotatedDataFrame', pData(nd)), annotation="FP-MaxLFQ-eof")

saveRDS(ndr, paste0(.res, "nd.eof.roastgsa.rds"))

#################################################################################################################
  # ROAST-GSA

rm(list=ls())
form <- readRDS(paste0(.res0, "G.Discovery_DIA_FP_MaxLFQ/G2.Diff_expression/form.rds"))
...
lgroups <- list("Mild-Asymptomatic"=list(list(group=c("Mild")), list(group=c("Asymptomatic"))),
                "Severe-Mild"=list(list(group=c("Severe")), list(group=c("Mild"))),
                "Critical-Severe"=list(list(group=c("Critical")), list(group=c("Severe"))))
lsigns <- sapply(1:length(lgroups), function(j) c(1, -1), simplify=F)
conts <- sapply(1:length(lsigns), function(j)
    make.contrasts(d=pData(nd), form=as.formula(form),
                   lsel=lgroups[[j]],
...
rm(list=ls())
lrg <- readRDS(file=paste0(.res, "lrgsa.maxmean.rds"))
lc <- readRDS(paste0(.res0, "G.Discovery_DIA_FP_MaxLFQ/G2.Diff_expression/lcomp.rds"))
...
for (o in names(lg[[1]]))
{
    links[[o]] <- paste0("rgsa_results/", rownames(d), "_", o, ".html")
    links[[paste0(o, "<br>heatmap")]] <- paste0("heatmaps_gsets/", rownames(d), "_", o, ".png")
    plots[[paste0(o, "<br>heatmap")]] <- paste0("heatmaps_gsets/", rownames(d), "_", o, ".png")
}
...
head(goTermsRes)
write.table(goTermsRes,paste0(.res,"GOterms.bp.mf.cc.allContrasts.csv"),
            sep=",")

goTermsRes.signif <- goTermsRes[goTermsRes$pvalue<0.05,]
summary(goTermsRes.signif)
write.table(goTermsRes.signif,paste0(.res,"GOterms.bp.mf.cc.allContrasts.signif.csv"),
            sep=",")
